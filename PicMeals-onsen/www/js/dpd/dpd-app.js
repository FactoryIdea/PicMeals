    ...
    function initialize()
    {
        ...
        dpd('users').get('me', function(result, error) {
          ...
          if (result.id!==undefined) {
                ...
                SetMenuAccount();
                GetMealsList();
                $('#mUserN').css('display','none');
                $('#mUserY').css('display','block');
                myNavigator.resetToPage('page_List.html', { animation : 'slide' });
              } 
        });  
        GetDataSettings();
    }
            ...
            dpd('users').get('?username=' + r.email , function(result, error) {
                if (result) {
                  ...
                  if (JSON.stringify(result)!=="[]") { 
                      _isFirstTime = false;
                      login(r.email, r.id + "_testApp");
                  } else 
                  {
                    _isFirstTime = true;
                    var obj = new Object();
                    obj.username = r.email;
                    obj.password = r.id + "_testApp";
                    obj.accountType = "U";
                    obj.email = r.email;
                    obj.img = r.thumbnail;
        ...
        dpd('users').post(o, function(data, err) {
              if (err) {
               ...
               if (data) {
                  ...
                  //alert(JSON.stringify(o.socialAccountId));
                   _UserRefID = data.id;
                   if (o.socialAccountId!==null)
                   {
                       login(o.email, o.socialAccountId + "_testApp");                  
                   }
                   else
                   {
        ...
        dpd('meals').get('?include=user&public=1&$limit=' + _itemPerPage + '&$skip=' + _SkipPage, function(data, err) {
              ...
              if (data.length) {
                $.each(data, function (i, item) {
			          $('#listGallery').append("<div class='item'><div class='box'><img src='http:" + item.img.replace("_300","_700") + "' class='det' id='" + item.user + "@" + item.id + "'/><p>" + item.title + "<br/><span class='badge'>" + item.likes + "</span> Yummies</p></div></div>");
			      });
                  //$('#pageLoader').modal('hide');
...
function activate_page(pname)
{
    //alert(pname);
    //menu.setMainPage( pname + '.html', { animation: "none"});
    myNavigator.pushPage(pname + '.html', { animation : 'slide' });
}